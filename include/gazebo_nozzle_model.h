...
static const std::string kDefaultNozzleFailureNumSubTopic = "/gazebo/nozzle_failure_num";
static const std::string kDefaultNozzleVelocityPubTopic = "/nozzle_speed";
...
class GazeboNozzleModel : public NozzleModel, public ModelPlugin {
 public:
  GazeboNozzleModel()
      : ModelPlugin(),
        NozzleModel(),
        command_sub_topic_(kDefaultCommandSubTopic),
        nozzle_failure_sub_topic_(kDefaultNozzleFailureNumSubTopic),
        nozzle_speed_pub_topic_(kDefaultNozzleVelocityPubTopic),
        nozzle_number_(0),
        nozzle_Failure_Number_(0),
 ...
 protected:
  ...
  virtual void UpdateNozzleFail();
  virtual void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf);
  virtual void OnUpdate(const common::UpdateInfo & /*_info*/);

 private:
  std::string command_sub_topic_;
  std::string nozzle_failure_sub_topic_;
  std::string joint_name_;
  std::string link_name_;
  std::string nozzle_speed_pub_topic_;
  std::string namespace_;

  int nozzle_number_;
  int turning_direction_;

  int nozzle_Failure_Number_; /*!< motor_Failure_Number is (motor_number_ + 1) as (0) is considered no_fail. Publish accordingly */
  int tmp_nozzle_num; // A temporary variable used to print msg
  ...
  transport::PublisherPtr nozzle_velocity_pub_;
  transport::SubscriberPtr command_sub_;
  transport::SubscriberPtr nozzle_failure_sub_; /*!< Subscribing to motor_failure_sub_topic_; receiving motor number to fail, as an integer */
  ...
  void NozzleFailureCallback(const boost::shared_ptr<const msgs::Int> &fail_msg);  /*!< Callback for the motor_failure_sub_ subscriber */
